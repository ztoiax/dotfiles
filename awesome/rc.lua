...
globalkeys = gears.table.join(globalkeys,
    ...
    -- awful.key({ mod1, "Control", "Shift" }, "a" , shell("scrot -s -f -q 100 --silent - | tesseract stdin stdout |xclip -selection clipboard")),
    -- awful.key({ mod1, "Control", "Shift"  }, "a" , shell("flameshot gui")),
    awful.key({ mod1, "Control", "Shift" }, "a" , shell("rm -rf ~/.cache/com.pot-app.desktop/pot_screenshot_cut.png && flameshot gui -s -p ~/.cache/com.pot-app.desktop/pot_screenshot_cut.png && curl '127.0.0.1:60828/ocr_recognize?screenshot=false'")),
    ...
    awful.key({ modkey, "Shift" }, "j" ,
              function ()
                  ...
                  if client.focus then
                      local tag = client.focus.screen.tags[current + 1]
                      if tag then
                          client.focus:move_to_tag(tag)
                      end
                 end
...
awful.rules.rules = {
    ...
    { rule = { class = "google-chrome-stable" }, properties = { screen = 1, tag = "1", floating = false }},
    { rule = { class = "microsoft-edge-stable" }, properties = { screen = 1, tag = "1", floating = false }},
}
-- }}}

-- awful.rules.rules[#awful.rules.rules+1] = { rule = { class = "netease-cloud-music" }, properties = { screen = 1, tag = 9, floating = false }}
-- t = {
...
--     awful.rules.rules[#t+1] = { rule = { class = key }, properties = { screen = 1, tag = value, floating = false }},
-- end
...
-- autostart
awful.spawn.with_shell("~/.dwm/autostart_blocking.sh")
awful.spawn.with_shell("~/.dwm/autostart.sh")
